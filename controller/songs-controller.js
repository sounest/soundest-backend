...
const deleteSong = async (req, res) => {
  try {
    ...
    if (!deletedSong) return res.status(404).json({ msg: "Song not found" });
...
const searchSongs = async (req, res) => {
  try {
    ...
    res.status(500).json({ msg: "Failed to search songs", error: err.message });
  }
};
...
const bulkInsertSongs = async (req, res) => {
  try {
    ...
    if (!Array.isArray(songs)) {
      return res
        .status(400)
        .json({ error: "Request body must be an array of songs" });
    }
...
const searchtrandingSongs = async (req, res) => {
  try {
    const results = await Song.find({
      description: { $regex: "tranding song", $options: "i" },
    });
